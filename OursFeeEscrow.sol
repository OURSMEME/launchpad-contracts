...
import {IOursFeeEscrow} from "./interfaces/ILaunchpad.sol";

/**
 * @title OursFeeEscrow
 * @notice Holds every recipient's claimable balance in one place, so
 ...
 * the post-graduation OursMemeHook credit the same way, so a recipient's
 * balance is denominated identically before and after graduation.
 */
contract OursFeeEscrow is ReentrancyGuard, IOursFeeEscrow {
