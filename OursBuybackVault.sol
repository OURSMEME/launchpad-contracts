...
import {IOursFeeEscrow, IOursFeePolicy, IOursLaunchFactory} from "./interfaces/ILaunchpad.sol";

/**
 * @title OursBuybackVault
 ...
 * "single deployment for every token" pattern OursFeeEscrow already uses.
 *
 ...
 */
contract OursBuybackVault is Ownable2Step, ReentrancyGuard {
    ...
    IOursFeePolicy public immutable feePolicy;
    IOursFeeEscrow public immutable feeEscrow;
     ...
     */
    constructor(address initialOwner, IOursFeePolicy feePolicy_, IOursFeeEscrow feeEscrow_) Ownable(initialOwner) {
    ...
    function _isAuthorizedLocker(address token, address caller) private view returns (bool) {
        ...
        return caller == IOursLaunchFactory(factory).getLaunchedToken(token).curve;
    }
}
