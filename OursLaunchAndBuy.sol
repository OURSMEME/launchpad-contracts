...
import {OursLaunchFactory} from "./OursLaunchFactory.sol";
import {OursBondingCurve} from "./OursBondingCurve.sol";

/**
 * @title OursLaunchAndBuy
 * @notice Creates a Ours launch and takes the first position on its curve
 * in a single transaction.
 *
 * OursLaunchFactory cannot fold a developer buy into `launchToken`, so a
 ...
 */
contract OursLaunchAndBuy is Ownable2Step, ReentrancyGuard {
    ...
    OursLaunchFactory public immutable factory;
    ...
    constructor(OursLaunchFactory factory_, address owner_) Ownable(owner_) {
    ...
    function launchAndBuy(
        OursLaunchFactory.TokenParams calldata params,
        ...
        if (nativeQuote) {
            tokensOut = OursBondingCurve(payable(curve)).buy{value: quoteIn}(quoteIn, minTokensOut, recipient);
        } else {
            IERC20(pairToken).forceApprove(curve, quoteIn);
            tokensOut = OursBondingCurve(payable(curve)).buy(quoteIn, minTokensOut, recipient);
